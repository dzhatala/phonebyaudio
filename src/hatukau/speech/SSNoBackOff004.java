...
public final class SSNoBackOff004 {
				...
				if (frameProcessed > 0|true) {// AFTER NIS
					...
					if (frameProcessed > 2 |true) {// begin averaging ...
						...
						if (frameProcessed > 5|true) {
							SSNoBackOff004.copyVector(YS_last_3,
									YS_last_2, FreqResol);
						}

						
						if (frameProcessed > 4|true) {
							SSNoBackOff004.copyVector(YS_last_1,
									YS_last_2, FreqResol);
						}
						if (frameProcessed > 3|true) {
							SSNoBackOff004.copyVector(YS, YS_last_1,
									FreqResol);
						...
						if (frameProcessed > 7|true) {
							...
							VADInfo info = SSNoBackOff004.vad(Y_last_1,
									N, NoiseCounter, NoiseMargin, HangOver,
									debug);
							info.NoiseCounter = NoiseCounter[0];
							

							SSNoBackOff004.substract(YS_last_1, N, D);
							if (!info.SpeechFlag) {
								// smoothing noise
								for (int i = 0; i < FreqResol; i++) {
							...
							SSNoBackOff004.zeroVector(OverLapper,W);
					...
					if (frameProcessed > 1|true) {
						SSNoBackOff004.copyVector(Y_last_1, Y_last_2,
								FreqResol);
					}
					// ok
					if (frameProcessed > 0|true) {
						SSNoBackOff004.copyVector(YPhase, YPhase_last_1,
								FreqResol);
						SSNoBackOff004.copyVector(Y, Y_last_1, FreqResol);
