...
public final class SSNoBackOff001 {
	...
	public static void main(String[] args) {
		...
		d("Usage: java. hatukau.speech.SSNoBackOff src.wav tgt.wav");
		System.exit(-1);
	}
		...
		try {
			...
			do {
				...
				if (frameProcessed > 0) {// AFTER NIS
					...
					if (frameProcessed > 0) {// begin averaging ...
						// if (frameProcessed == 1)
						// d("Begin magnitude average");
						/** Magnitude Average finding YS **/

						if (frameProcessed > 0) {
							SSNoBackOff001.copyVector(YS_last_3,
									YS_last_2, FreqResol);
						}

						
						if (frameProcessed > 0) {
							SSNoBackOff001.copyVector(YS_last_1,
									YS_last_2, FreqResol);
						}
						if (frameProcessed > 0) {
							SSNoBackOff001.copyVector(YS, YS_last_1,
									FreqResol);
						...
						if (frameProcessed > 7) {
							...
							VADInfo info = SSNoBackOff001.vad(Y_last_1,
									N, NoiseCounter, NoiseMargin, HangOver,
									debug);
							info.NoiseCounter = NoiseCounter[0];
							

							SSNoBackOff001.substract(YS_last_1, N, D);
							if (!info.SpeechFlag) {
								// smoothing noise
								for (int i = 0; i < FreqResol; i++) {
							...
							SSNoBackOff001.zeroVector(OverLapper,W);
					...
					if (frameProcessed > 0) {
						SSNoBackOff001.copyVector(Y_last_1, Y_last_2,
								FreqResol);
					}
					// ok
					if (frameProcessed > 0) {
						SSNoBackOff001.copyVector(YPhase, YPhase_last_1,
								FreqResol);
						SSNoBackOff001.copyVector(Y, Y_last_1, FreqResol);
		...
		try {
			...
			do {
				...
				if (frameProcessed > 0) {// AFTER NIS
					...
					// ok
					if (frameProcessed > 0) {
						SSNoBackOff001.copyVector(YPhase, YPhase_last,
								FreqResol);
						SSNoBackOff001.copyVector(Y, Y_last_1, FreqResol);
