...
public class NewWavFile
{
	 ...
	 * @param file
	 * @param numChannels
	 * @param numFrames desired numFrames, remember to padding to complete before close
	 * , or error in HTK
	 * @param validBits
	 * @param sampleRate
	 * @return
	 ...
	 */
	public static NewWavFile newWavFile(File file, int numChannels, long numFrames, int validBits, long sampleRate) throws IOException, WavFileException
	{
		// Instantiate new Wavfile and initialise
		NewWavFile wavFile = new NewWavFile();
		wavFile.file = file;
	...
	public static NewWavFile openWavFile(File file) throws IOException, WavFileException
	{
		...
		while (true)
		{
			...
			if (chunkID == FMT_CHUNK_ID)
			{
				...
				if (wavFile.bytesPerSample * wavFile.numChannels != wavFile.blockAlign)
					throw new WavFileException("Block Align "+wavFile.blockAlign + " !="+(wavFile.bytesPerSample * wavFile.numChannels) +" does not agree with bytes required for validBits and number of channels");
	...
	public void incNumFrames2(long l){
		numFrames=l;
	...
	public void display(PrintStream out)
	{
		...
		out.printf("floatScale %1.3f, floatOffset %1.3f\n", floatScale, floatOffset);
		out.printf("wordalign %s\n", wordAlignAdjust?"true":"false");
	...
	public static void main(String[] args)
	{
		...
		try
		{
			...
			{
				NewWavFile readWavFile = openWavFile(new File(filename));
				readWavFile.display();
				...
				NewWavFile writeWavFile = newWavFile(new File("out.wav"), numChannels, numFrames, validBits, sampleRate);
...
//				long[] buffer = new long[BUF_SIZE * numChannels];
			...
			NewWavFile writeWavFile = newWavFile(new File("out2.wav"), 1, 10, 23, 44100);
