...
public final class SSNoBackOff002 {


	private static int totalWritten;
	...
	public static void perform(String ssrc, String stgt) {
		try {
			...
			NewWavFile nWavFile = NewWavFile.newWavFile(new File(stgt), channels,
					numFrames-sampleRate/2, validBits, sampleRate);
			//d("window W= " + W);
			...
			do {
				...
				if (frameProcessed > 0) {// AFTER NIS
					...
					if (frameProcessed > 2) {// begin averaging ...
						...
						if (frameProcessed > 5) {
							SSNoBackOff002.copyVector(YS_last_3,
									YS_last_2, FreqResol);
						}

						
						if (frameProcessed > 4) {
							SSNoBackOff002.copyVector(YS_last_1,
									YS_last_2, FreqResol);
						}
						if (frameProcessed > 3) {
							SSNoBackOff002.copyVector(YS, YS_last_1,
									FreqResol);
						...
						if (frameProcessed > 7) {
							...
							VADInfo info = SSNoBackOff002.vad(Y_last_1,
									N, NoiseCounter, NoiseMargin, HangOver,
									debug);
							info.NoiseCounter = NoiseCounter[0];
							

							SSNoBackOff002.substract(YS_last_1, N, D);
							if (!info.SpeechFlag) {
								// smoothing noise
								for (int i = 0; i < FreqResol; i++) {
							...
							SSNoBackOff002.zeroVector(OverLapper,W);
					...
					if (frameProcessed > 1) {
						SSNoBackOff002.copyVector(Y_last_1, Y_last_2,
								FreqResol);
					}
					// ok
					if (frameProcessed > 0) {
						SSNoBackOff002.copyVector(YPhase, YPhase_last_1,
								FreqResol);
						SSNoBackOff002.copyVector(Y, Y_last_1, FreqResol);
	...
	/**
	 * perform spectral substraction of files
	 * 
	 * @param source
	 *            and save result in tgt
	 */
	public static void performold(String source, String tgt) {
		try {
			// Open the wav file specified as the first argument
			d(".perform " + source + "-->" + tgt);
			NewWavFile wavFile = NewWavFile.openWavFile(new File(source));
			...
			int channels = wavFile.getNumChannels();
			int validBits = wavFile.getValidBits();
			WavFile nWavFile = WavFile.newWavFile(new File(tgt), channels,
			...
			do {
				...
				if (!NIS_RETRACE | NIS_RETRACE_FRAME_NO >= NIS) {
					...
					if (TotalYFrameRead == 1) {
						framesRead = 0;
						try {
							framesRead = wavFile.readFrames(afterwsBuffer, W
									- wSP); // read
											// a
				...
				if (frameProcessed > 0) {// AFTER NIS
					...
					// ok
					if (frameProcessed > 0) {
						SSNoBackOff002.copyVector(YPhase, YPhase_last,
								FreqResol);
						SSNoBackOff002.copyVector(Y, Y_last_1, FreqResol);
			...
			if (error_read) {
				while (nWavFile.getFramesRemaining() > 0) {
					totalWritten+=nWavFile.writeFrames(output, wSP);// padding with last noise
				}
				d(" writing total "+ nWavFile.getNumFrames());
			}
